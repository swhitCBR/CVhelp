...
get_env_plot <- function(
  ...
  ggplot2::ggplot() + 
    ...
    ggplot2::geom_line(data=env_data_long_in,ggplot2::aes(x=DOY,y=value,color=Year,linewidth=yr_high_lb)) +
    ggplot2::geom_rug(data=env_data_long_in %>% filter(is.na(value)),ggplot2::aes(x=DOY),color="black",linewidth = 1.5) +
    ggplot2::geom_rug(data=env_data_long_in %>% filter(is.na(value)),ggplot2::aes(x=DOY,color=Year)) +
